...
const createStatic = async (req, res, next) => {
    ...
    try {
        ...
        return next(new ApiError(`Error From Create Static: ${error.message}`, 500));
    }
};
...
const updateElement = async (req, res, next) => {
    ...
    try {
        ...
        let oldData = await StaticData.findById(id);
        if (!oldData) {
            return res.status(404).json({ status: "Fail", message: `لا توجد بيانات بهذا الـ ID: ${id}` });
        }
        ...
        if (req.files?.categoriesHeroURL) deleteOldFiles([oldData.categoriesHeroURL]); // حذف ملف الـ categoriesHeroURL القديم
        ...
        let updatedData = {
            ...newData,
            logoURL: req.files?.logoURL ? `uploads/${req.files.logoURL[0].filename}` : oldData.logoURL,
            homeHeroURL: req.files?.homeHeroURL ? `uploads/${req.files.homeHeroURL[0].filename}` : oldData.homeHeroURL,
            servicesHeroURL: req.files?.servicesHeroURL ? `uploads/${req.files.servicesHeroURL[0].filename}` : oldData.servicesHeroURL,
            categoriesHeroURL: req.files?.categoriesHeroURL ? `uploads/${req.files.categoriesHeroURL[0].filename}` : oldData.categoriesHeroURL,
            aboutHeroURL: req.files?.aboutHeroURL ? `uploads/${req.files.aboutHeroURL[0].filename}` : oldData.aboutHeroURL
        };

        // تحديث البيانات في قاعدة البيانات
        let data = await StaticData.findByIdAndUpdate(id, updatedData, { new: true, runValidators: false });
...
export {
    createStatic , getAllStatic ,getElementById  ,updateElement
}