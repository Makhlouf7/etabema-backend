...
const createOrder = async (req, res, next) => {
  try {
    ...
    emailOptions(newOrder.email, newOrder.title, newOrder.status, newOrder._id);
  } catch (error) {
    console.log(error);
...
const updateOrder = async (req, res, next) => {
  ...
  try {
    ...
    let data = await Order.findByIdAndUpdate(id, { ...newData }, { new: true });
    res.status(200).json({ status: "Success", data: data });

    emailOptions(data.email, data.title, data.status, data._id);
  } catch (error) {
    console.log(error);
